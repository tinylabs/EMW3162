...
while opt=$(zenity --width 309 --height 300 --title="$title" --text="$prompt" --list \
 ...
 case $opt in
    "${options[0]}" ) 
        pid="gst-launch udpsrc uri=\"udp://192.168.0.2:50007\" buffer-size=200 ! audioparse rate=22050 signed=true width=16 depth=16 format=int channels=1 ! alsasink sync=false"
        echo $pid
        exec $pid &
        ;;
    "${options[1]}") 
		pid="gst-launch udpsrc uri=\"udp://192.168.0.2:50007\" buffer-size=2000 ! audioparse rate=22050 signed=true width=16 depth=16 format=int channels=1 ! filesink location=dump.bin"
		echo $pid
		exec $pid &
        ;;
    "${options[2]}") 
